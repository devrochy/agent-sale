...
- **Límite conocido, resuelto en la Fase 2 del plan de remediación del incidente 2026-09-13** (ver `src/orchestrator/debounceFailures.ts` y `migrations/0062_debounce_failures.cjs`): si `processConversation` o el envío fallan cuando el timer vence (`fireConversation` en `debounceScheduler.ts`), no hay backing de Redis Streams para reintentar automáticamente (el mensaje original ya se hizo `XACK` al ingerirse) — pero ahora `fireConversation` sí reintenta una vez más (con ~2s de espera) antes de darse por vencido, y si el segundo intento también falla, el fallo queda registrado en `debounce_failures` (consultable por SQL) en vez de perderse solo en el log. El reintento repite el turno completo, no solo el envío: es seguro porque `crear_pedido`/`agregar_item_pedido` tienen idempotency_key atado al `messageSid` (que no cambia entre intentos) y `crear_pedido` chequea `quote_id` duplicado antes de llamar a Wompi o insertar. El mensaje del cliente sigue guardado y visible en el inbox de Conversaciones; el barrido de recuperación del próximo arranque lo detectaría igual si el proceso se reinicia a mitad de los reintentos.
