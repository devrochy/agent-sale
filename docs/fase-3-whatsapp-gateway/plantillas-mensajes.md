...
|---|---|---|---|
| `pedido_confirmado_v3` | Utility | `enviarPedidoConfirmado.ts`, llamado desde `confirmarDomicilioPedido.ts` (tool `confirmar_domicilio_pedido`), `actualizarDireccionPedido.ts` (tool `actualizar_direccion_pedido`) y `confirmarDomicilioManual` (panel) — siempre DESPUÉS de que la dirección queda confirmada, nunca junto con `confirmar_domicilio` | No revierte la confirmación/cambio de dirección; queda reflejado en `pedido_confirmado_status: "plantilla_no_aprobada"` en la respuesta de esas tools |
| `metodo_pago` | Utility | `preguntarMetodoPago.ts` (tool `preguntar_metodo_pago`) | Ídem, sin fallback de texto libre |
| `confirmar_domicilio` | Utility | `confirmarDomicilioPedido.ts` (`pedirConfirmacionDomicilio`, tool `pedir_confirmacion_domicilio` — primer paso del cierre) + `reenviarConfirmacionDomicilio` (reenvío desde el panel) | `pedir_confirmacion_domicilio` devuelve `plantilla_no_aprobada` y el LLM sigue por texto libre; el panel puede reenviarla después o el admin confirma a mano (`confirmarDomicilioManual`) — si no está aprobada tampoco tiene sentido usar `actualizar_direccion_pedido` para este paso |
...
- **Código:** `src/domains/commerce/enviarPedidoConfirmado.ts` (mandado desde `confirmarDomicilioPedido.ts`/`actualizarDireccionPedido.ts`/panel — ver `confirmar_domicilio` más abajo); el botón `Confirmar y pagar` lo resuelve `src/domains/commerce/confirmarPagoPedido.ts` (tool `confirmar_pago_pedido`).
...
- **Código:** `src/domains/commerce/confirmarDomicilioPedido.ts` — `pedirConfirmacionDomicilio` (tool `pedir_confirmacion_domicilio`, primer paso del cierre) manda el envío inicial; `reenviarConfirmacionDomicilio` es el reenvío manual desde el panel. Los 3 botones se resuelven en `confirmarDomicilioPedido` (tool `confirmar_domicilio_pedido`, botón "Confirmar dirección") y `src/domains/commerce/actualizarDireccionPedido.ts` (tool `actualizar_direccion_pedido`, botones de cambio — actualiza `orders.delivery_address` siempre y `customers.address` solo si el cliente tocó "Cambiar permanentemente", mismo criterio que `save_permanently` en `crearPedido.ts`). **Los 3 botones, apenas resuelven, mandan a su vez `pedido_confirmado_v3`** (ver `enviarPedidoConfirmado.ts`) — es el paso siguiente del flujo, ya no va junto con este envío.
