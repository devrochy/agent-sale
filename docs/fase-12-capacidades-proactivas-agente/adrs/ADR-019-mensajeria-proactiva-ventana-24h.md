...
### Nota post-implementación (Reporte diario, Fase 12.2)

Esta ADR clasificó "dentro/fuera de la ventana" únicamente para mensajes a **clientes** (que ya están en una conversación activa con el bot). El Reporte diario (#7) expuso un caso que esta ADR no había considerado: va al **operador/dueño** del negocio, cuyo número no tiene ninguna garantía de estar dentro de la ventana de 24h — nadie lo obliga a "iniciar conversación" primero, a diferencia de un cliente.

Confirmado en QA real: un envío a un número que nunca le había escrito al bot quedó `status: "undelivered"` (Twilio `errorCode` 63016 — "fuera de la ventana de 24h, se necesita un Message Template"), aceptado por la API sin lanzar ningún error, invisible hasta consultar el estado del mensaje después. `src/gateway/sendMessage.ts` (`getWhatsAppMessageStatus`) y `src/jobs/dailyReport.ts` (`verifyDelivery`) ahora consultan ese estado tras un delay corto y lo loguean explícitamente si quedó rechazado — no evita el rechazo (seguiría necesitando una plantilla aprobada por Meta para garantizar entrega sin depender de que el operador le escriba primero), pero al menos deja de fallar en silencio.
