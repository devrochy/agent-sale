...
**Dos estimaciones de esta tabla se revisan a la baja** en [comparativa-arquitectura-forja.md](../comparativa-arquitectura-forja.md#qué-proponemos-como-mejora-futura-con-esfuerzo-revisado-a-la-baja-frente-a-estimaciones-previas): Cobros (#12) puede reducirse a solo generar un link a un Payment Link de Stripe pre-configurado, sin webhook de confirmación; y Reporte diario (#7) puede empezar como comando bajo demanda en el panel, sin esperar a `node-cron` (ADR-018).
...
- **Multimodalidad y cobros** quedan fuera de esta fase deliberadamente — incluirlas de forma superficial aquí sería subestimar su esfuerzo real (integraciones externas completas, no extensiones del código actual).
