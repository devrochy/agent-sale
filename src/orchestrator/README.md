...
Escalar (`escalar_a_humano`, palabras clave, guardrails, monto alto, intentos fallidos) crea un ticket en `handoff_queue` y marca `conversations.state.step = "escalado"` como etiqueta/auditoría, pero **no** silencia al agente: el bot sigue respondiendo normalmente turno a turno hasta que un admin efectivamente toma el ticket desde el panel, momento en el que recién se pone `conversations.bot_paused = true` (ver `admin/adminPanel.ts` → `tomarTicket`, chequeado en `consumer.ts` antes de invocar el resto del pipeline). La vista del asesor (para tomar/cerrar el caso) es un incremento de código separado.
